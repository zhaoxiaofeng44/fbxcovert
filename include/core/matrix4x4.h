...
public:
	...
	Matrix4x4(const Quaternion &quaternion);
	~Matrix4x4() {};
	...
	void            copy(double *other) const;

	vec3f			getTransform() const;
	vec3f			getScale() const;
	Quaternion		getQuaternion() const;

	void setTRS(const vec3f& translation, const Quaternion& rotation, const vec3f& scale)
	{
		Matrix4x4 rot3x3(rotation);
	...
	static void	copy(double *other, const Matrix4x4 &mat);
...
inline Matrix4x4::Matrix4x4(const Quaternion &quaternion)
{
	//http://www.j3d.org/matrix_faq/matrfaq_latest.html#Q56
	float x2 = 2.0f * quaternion.v.x;
	float y2 = 2.0f * quaternion.v.y;
	float z2 = 2.0f * quaternion.v.z;

	float xx2 = x2 * quaternion.v.x;
	float xy2 = x2 * quaternion.v.y;
	float xz2 = x2 * quaternion.v.z;
	float xw2 = x2 * quaternion.w;

	float yy2 = y2 * quaternion.v.y;
	float yz2 = y2 * quaternion.v.z;
	float yw2 = y2 * quaternion.w;

	float zz2 = z2 * quaternion.v.z;
	float zw2 = z2 * quaternion.w;
...
inline void  Matrix4x4::copy(double *other) const
{
	copy(other,*this);
}
...
inline void	 Matrix4x4::copy(double *other, const Matrix4x4 & mat)
{
	other[0] = mat[0][0];
	other[1] = mat[0][1];
	other[2] = mat[0][2];
